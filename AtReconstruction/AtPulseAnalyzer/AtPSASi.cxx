...
void AtPSASi::SetFitClipped(bool fit) { fFitClipped = fit; }

void AtPSASi::SetDumpTraces(bool dump) { 
  if (dump) { 
    std::cout << "Warning! SetDumpTraces will make potentially very large ASCII files! This should only be used for debugging purposes!" << std::endl;
  }
  fDumpTraces = dump; 
}

void AtPSASi::SetOverflowReconstruction(bool overflow) { fOverflowReconstruction = overflow; }
...
AtPSASi::HitVector AtPSASi::AnalyzeGenTrace(AtGenericTrace *genTrace)
{
   ...
   float precount = 0;
   float postcount = 0;
   for (int i = std::max(fMinIndx, maxAdcIdx + fLowBLRegion[0]); i < std::min(maxAdcIdx + fLowBLRegion[1], (int)(floatADCVector.size()-12));
        i++) { // more baseline added
      baseline += floatADC[i];
   ...
   for (int i = std::max(fMinIndx, maxAdcIdx + fHighBLRegion[0]); i < std::min(maxAdcIdx + fHighBLRegion[1], (int)(floatADCVector.size()-12));
        i++) { // more baseline added
      baseline += floatADC[i];
   ...
   if (saturation && fFitClipped) { //fit the good part of the trace to extrapolate the peak
     ...
     TGraph gr;
     int n = 0;
     for (int i = std::max(fMinIndx, min_overflow -4); i < std::min(max_overflow+7, (int)(floatADCVector.size()-12));
         i++) { // more baseline added
       if (floatADC[i] < 8191) { //overflow hack
     ...
     gr.Fit(&func,   "", "", 0, 256);
     std::cout << "saturated GAGG fit to " << func.GetParameter(1) << "  " << func.GetParameter(3) << "  " << func.GetParameter(4) << "  " <<  func.GetMaximum() << std::endl;
     if (fDumpTraces) {
